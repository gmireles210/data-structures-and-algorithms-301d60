...
const findAnything = (arr, target) => {
  return arr.reduce((arrayNew, arrayC) => {
    arrayC.includes (target) ? arrayNew.push(arrayC) : arrayNew;
...
const unenrollBrook = (arr) => {
  return arr.reduce((arrayOut, arrayNow) => {
    let arrayNowProduct = arrayNow.filter(string => !string.includes('Brook'));
    arrayOut.push(arrayNowProduct);
    return arrayOut; }, [])
