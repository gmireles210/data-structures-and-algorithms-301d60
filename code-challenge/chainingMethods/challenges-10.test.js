...
const count = (target, input) => {
  // flattened the array
  var flat = input.reduce((accum, value) => {
    return accum.concat(value);
  },[]);

  // count the target
  var counter = flat.reduce((accum, value) => {
    if(value === target){
      accum +=1;
    }
    return accum;
...
const totalSum = (input) => {
  ...
  const flatten = input.reduce((accum, value) => {
    return accum.concat(value);
  },[]);

  const sum = flatten.reduce((accum, value) => {
    return accum += value;
  },0);
  return sum;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  const answer = input.map (ele => {
    const removal = ele.reduce ( (answerThusFar, value) => {
      if(value % 5 === 0 && typeof value !== 'string') {
        answerThusFar.push(Math.pow(2, value))
      }
...
returns the namesGiven of the characters whose gender is either male or female.
The namesGiven should be combined into a single string with each character name separated by "and".
...
let findMaleAndFemale = (data) => {
  ...
  let namesGiven = data.reduce ((answerThusFar, value) => {
    if(value.gender === 'male' || value.gender === 'female'){
      answerThusFar.push(value.name);
    }
    return answerThusFar;
  }, []);
  return (namesGiven.join(' and '));
};
...
let findShortest = (data) => {
  // Solution code here...
  let nameShort;
  let shortest = data.reduce ((answerThusFar, value) =>
    (value.height > answerThusFar ? value.height : answerThusFar),
  data[0].height
  );
