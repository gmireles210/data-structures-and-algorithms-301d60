...
const countNumberOfElements = (arr) => {
  let sum = arr.reduce ((answerNow, value, idx) => {
    answerNow += 1;
...
const returnNames = (arr) => {
  let names = arr.reduce ((answerNow, value, idx) => {
    answerNow.push (value.name);
    return answerNow;
  }, []);
  return names;
};
...
const reversedString = (str) => {
  let arr = str.split('');
  let reverseArr = arr.reduce ((answerNow,value)=>{
    answerNow.unshift (value);
    return answerNow;
  }, [])
...
const countNumberOfChildren = (arr) => {
  let childrenCount = characters.reduce((answerNow, value) => {
    if(value.children){
      answerNow += value.children.length;
    }
    return answerNow;
  },0);
...
const calculateAverage = (arr) => {
  return arr.reduce((answerNow, value) => (answerNow + value)) / arr.length;
};
